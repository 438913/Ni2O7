...
def check_in_vs_condition1(x1, y1, x2, y2, x3, y3, x4, y4):
    ...
    if (check_in_vs_condition(x1, y1) and check_in_vs_condition(x2, y2) and check_in_vs_condition(x3, y3)
            and check_in_vs_condition(x4, y4)):
        return True
    else:
        return False
...
def check_Pauli(slabel):
    ...
    :param slabel: 4 holes state
    :return: logic (True or False)
    """
    logic = True
    length = len(slabel)
    for i in range(0, length - 5, 5):  # length = 20, i = 0, 5, 10
        if not logic:
            break
        s1, orb1, x1, y1, z1 = slabel[i: i + 5]
        for j in range(i + 5, length, 5):  # when i = 0, j = 5, 10, 15; when i = 5, j= 10, 15; ...
...
def exist_d6_d7_state(o1, o2, o3, o4, x1, x2, x3, x4):
    if (o1 in pam.Ni_orbs and o2 in pam.Ni_orbs and o3 in pam.Ni_orbs and x1 == x2 == x3) or \
            ...
            (o2 in pam.Ni_orbs and o3 in pam.Ni_orbs and o4 in pam.Ni_orbs and x2 == x3 == x4):
        return False
    else:
        return True


def exist_d6_state(o1, o2, o3, o4, x1, x2, x3, x4):
...
class VariationalSpace:
                                    ...
                                    for f1_w in funlist_w[0]:
                                        ...
                                        for f1 in funlist[0]:
                                            ...
                                            for f2 in funlist2[0]:
                                                ...
                                                # neglect d7 state !!
                                                if not exist_d6_state(orb1, orb2, orb3, orb4, ux, vx, tx, wx):
                                                    continue
    ...
    def get_uid(self, state):
        ...
        uid = i1 + 2 * i2 + 4 * i3 + 8 * i4 + 32 * z1 + 64 * z2 + 128 * z3 + 256 * z4 + 1024 * o1 + 1024 * N * o2 + 1024 * N2 * o3 + 1024 * N3 * o4 + 1024 * N5 * (
                (y1 + s) + (x1 + s) * B1 + (y2 + s) * B2 + (x2 + s) * B3 + (y3 + s) * B4 + (x3 + s) * B5 + (y4 + s) * B6 + (x4 + s) * B7)
        #         print (state)
        ...
        assert ((s1, orb1, x1, y1, z1, s2, orb2, x2, y2, z2, s3, orb3, x3, y3, z3, s4, orb4, x4, y4, z4) == (ts1, torb1, tx1, ty1, tz1, ts2, torb2, tx2, ty2, tz2, ts3, torb3, tx3, ty3, tz3, ts4, torb4, tx4, ty4,
                 tz4))
        return uid
