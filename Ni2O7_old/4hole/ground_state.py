...
def state_classification(state_param):
    ...
    :param state_param: state_param = [(orb1, x1, y1), (orb2, x2, y2), (orb3, x3, y3), (orb4, x4, y4)]
...
def get_ground_state(matrix, VS, S_Ni1_val, Sz_Ni1_val, S_Ni2_val, Sz_Ni2_val, bonding_val, U_Ni1, U_Ni2):
    ...
    txt.write('lowest eigenvalue of H = ' + '\r')
    txt.write(f'{vals}\r')
    ...
    for istate in ilead:
        ...
        input_state = [(orb1, x1, y1), (orb2, x2, y2), (orb3, x3, y3), (orb4, x4, y4)]
        ...
        if state_type_key in state_type_weight:
            ...
            type_specific_weight[state_type_key] += [istate]
        else:
            state_type_weight[state_type_key] = state_type_value
            type_specific_weight[state_type_key] = [istate]

    weight_list = sorted(state_type_weight.items(), key=lambda item: item[1], reverse=True)
    type_distribution = {'0.008 ~ 0.010': 0, '0.006 ~ 0.008': 0, '0.004 ~ 0.006': 0, '0.002 ~ 0.004': 0, '< 0.002': 0}
    other_type_weight = 0
    ...
    for state_type, type_weight in weight_list:
        ...
        other_weight = 0
        # specific_weight = sorted(type_specific_weight[state_type], key=lambda x: weights_average[x],  reverse=True)
        for istate in type_specific_weight[state_type]:
            weight = weights_average[istate]
            state = VS.get_state(VS.lookup_tbl[istate])
            s1 = state['hole1_spin']
            s2 = state['hole2_spin']
        ...
        txt1.write(f'\tother_distribution:  {other_distribution}\r\tother weight = {other_weight}\r\r')
