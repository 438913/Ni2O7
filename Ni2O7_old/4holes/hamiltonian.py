...
def get_double_occu_list(VS):
    ...
    '''
    dim = VS.dim
    d_Ni1_list = [];idx_Ni1 = [];hole34_Ni1_part = [];double_Ni1_part = [];
    d_Ni2_list = [];idx_Ni2 = [];hole34_Ni2_part = [];double_Ni2_part = [];
    p_list = [];apz_list = []
    ...
    return d_Ni1_list, idx_Ni1, hole34_Ni1_part, double_Ni1_part, \
        d_Ni2_list, idx_Ni2, hole34_Ni2_part, double_Ni2_part, \
        p_list


def create_interaction_matrix_ALL_syms(VS, d_double, double_part, idx, hole34_part, S_val, Sz_val, AorB_sym,
                                       A):
    '''
        ...
        for i, double_id in enumerate(d_double):
            ...
            for idx2, o34 in enumerate(sym_orbs):
                ...
                for s1 in ('up', 'dn'):
                    for s2 in ('up', 'dn'):
                        if idx[i] == 34:
                            slabel = [s1, o34[0]] + dpos + [s2, o34[1]] + dpos + hole34_part[i][0:10]
                        if idx[i] == 24:
                        ...
                        if idx[i] == 23:
                            slabel = [s1, o34[0]] + dpos + hole34_part[i][0:10] + [s2, o34[1]] + dpos
                        if idx[i] == 13:
                            slabel = hole34_part[i][0:5] + [s1, o34[0]] + dpos + hole34_part[i][5:10] + [s2, o34[1]] + dpos
                        if idx[i] == 12:
                            slabel = hole34_part[i][0:10] + [s1, o34[0]] + dpos + [s2, o34[1]] + dpos
