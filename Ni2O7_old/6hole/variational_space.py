...
def make_state_canonical(state):
    ...
    '''
    For 5 holes,to generate the canonical_state:
    ...
    '''
        For 6 holes,to generate the canonical_state:
    '''
            ...
            if tmp36 == tlabel:
                ...
                if tmp26 == tlabel:
                    ...
                    if tmp16 == tlabel:
                        ...
                        slabel4 = [s6, orb6, x6, y6, z6] + slabel3[0:25]
...
def check_in_vs_condition(x1, y1):
    ...
    '''
    if ((x1, y1) == (-2, 0) or (x1, y1) == (-1, 0) or (x1, y1) == (0, 0) or (x1, y1) == (1, 0) or (x1, y1) == (2, 0)
            or (x1, y1) == (-1, 1) or (x1, y1) == (-1, -1) or (x1, y1) == (1, 1) or (x1, y1) == (1, -1)):
        return True
    else:
        return False
...
def check_in_vs_condition1(x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6):
    ...
    and two-hole distance less than cutoff Mc
    '''
    if (check_in_vs_condition(x1, y1) and check_in_vs_condition(x2, y2) and check_in_vs_condition(x3, y3)
            and check_in_vs_condition(x4, y4) and check_in_vs_condition(x5, y5)) and check_in_vs_condition(x6, y6):
        return True
    else:
        return False
...
def check_Pauli(slabel):
    """
    Check if satisfy the Pauli principle
    :param slabel: 6 holes state
    :return: logic (True or False)
    """
    logic = True
    length = len(slabel)
    for i in range(0, length - 5, 5):  # length = 30, i = 0, 5, 10, 15, 20
        if not logic:
            break
        s1, orb1, x1, y1, z1 = slabel[i: i + 5]
        for j in range(i + 5, length, 5):  # when i = 0, j = 5, 10, 15, 20, 25; when i = 5, j= 10, 15, 20, 25; ...
            s2, orb2, x2, y2, z2 = slabel[j:  j + 5]
            if (s1, orb1, x1, y1, z1) == (s2, orb2, x2, y2, z2):
                logic = False
                break
    return logic
...
def exist_d6_d7_state(o1, o2, o3, o4, o5, o6, x1, x2, x3, x4, x5, x6):
    logic = True
    o_xs = combinations([(o1, x1), (o2, x2), (o3, x3), (o4, x4), (o5, x5), (o6, x6)], 3)
    for o_x in o_xs:
        (o1, x1), (o2, x2), (o3, x3) = o_x
...
def exist_d6_state(o1, o2, o3, o4, o5, o6, x1, x2, x3, x4, x5, x6):
    logic = True
    o_xs = combinations([(o1, x1), (o2, x2), (o3, x3), (o4, x4), (o5, x5), (o6, x6)], 4)
    for o_x in o_xs:
        (o1, x1), (o2, x2), (o3, x3), (o4, x4) = o_x
        if o1 in pam.Ni_orbs and o2 in pam.Ni_orbs and o3 in pam.Ni_orbs and o4 in pam.Ni_orbs and x1 == x2 == x3 == x4:
            logic = False
            break
    return logic
...
class VariationalSpace:
    ...
    def print_VS(self):
        for i in range(0, self.dim):
            ...
            print(i, ts1, torb1, tx1, ty1, tz1, ts2, torb2, tx2, ty2, ts3, torb3, tx3, ty3, tz3, ts4,
                  torb4, tx4, ty4,tz4, ts5, torb5, tx5, ty5, tz5, ts6, torb6, tx6, ty6, tz6)
                                    ...
                                    for f1_w in funlist_w[0]:
                                        ...
                                        for px in range(-Mc, Mc + 1):
                                            ...
                                            for f1_p in funlist_p[0]:
                                                ...
                                                for qx in range(-Mc, Mc + 1):
                                                    Bq = Mc - abs(qx)
                                                    funlist_q = [util.lamlist1(range(-Bq, Bq + 1), [0])]
                                                    for f1_q in funlist_q[0]:
                                                        qy, qz = f1_q()
                                                        orb6s = lat.get_unit_cell_rep(qx, qy, qz)
                                                        if orb6s == ['NotOnSublattice']:
                                                            continue
                                                        if not check_in_vs_condition1(ux, uy, vx, vy, tx, ty, wx, wy, px, py, qx, qy):
                                                            continue
                                                        ...
                                                        for f1 in funlist[0]:
                                                            ...
                                                            funlist2 = [util.lamlist(['up', 'dn'], ['up', 'dn'], ['up', 'dn'],
                                                                                     ['up', 'dn'], ['up', 'dn'], ['up', 'dn'])]
                                                            for f2 in funlist2[0]:
                                                                ...
                                                                # neglect d7 state !!
                                                                if not exist_d6_state(orb1, orb2, orb3, orb4, orb5, orb6, ux, vx, tx, wx, px, qx):
                                                                    continue


                                                                    # consider Pauli principle
                                                                slabel = [s1, orb1, ux, uy, uz, s2, orb2, vx, vy, vz, s3, orb3,tx, ty, tz, \
                                                                          s4, orb4, wx, wy, wz, s5, orb5, px, py, pz, s6, orb6, qx, qy, qz]
    ...
    def get_uid(self, state):
        ...
        uid = (i1 + 2 * i2 + 4 * i3 + 8 * i4 + 16 * i5 + 32 * i6 + 64 * z1 + 128 * z2 + 256 * z3 + 512 * z4 + 1024 * z5 + 2048 * z6
               + 4096 * o1 + 4096 * N * o2 + 4096 * N2 * o3 + 4096 * N3 * o4 + 4096 * N4 * o5 + 4096 * N5 * o6
               + 4096 * N6 * ((y1 + s) + (x1 + s) * B1 + (y2 + s) * B2 + (x2 + s) * B3 + (y3 + s) * B4 + (x3 + s) * B5
                              + (y4 + s) * B6 + (x4 + s) * B7 + (y5 + s) * B8 + (x5 + s) * B9 + (y6 + s) * B10 + (x6 + s) * B11))
        #         print (state)
        ...
        assert ((s1, orb1, x1, y1, z1, s2, orb2, x2, y2, z2, s3, orb3, x3, y3, z3, s4, orb4, x4, y4, z4, s5, orb5, x5,
                 y5, z5, s6, orb6, x6, y6, z6) == (ts1, torb1, tx1, ty1, tz1, ts2, torb2, tx2, ty2, tz2, ts3, torb3, tx3, ty3, tz3, ts4, torb4, tx4, ty4,
                 tz4, ts5, torb5, tx5, ty5, tz5, ts6, torb6, tx6, ty6, tz6))
        return uid
    ...
    def get_state(self, uid):
        ...
        x6 = int(uid / (4096 * N6 * B11)) - s
        uid_ = uid % (4096 * N6 * B11)
        y6 = int(uid_ / (4096 * N6 * B10)) - s
        uid_ = uid_ % (4096 * N6 * B10)
        x5 = int(uid_ / (4096 * N6 * B9)) - s
        uid_ = uid % (4096 * N6 * B9)
        y5 = int(uid_ / (4096 * N6 * B8)) - s
        uid_ = uid_ % (4096 * N6 * B8)
        x4 = int(uid_ / (4096 * N6 * B7)) - s
        uid_ = uid_ % (4096 * N6 * B7)
        y4 = int(uid_ / (4096 * N6 * B6)) - s
        uid_ = uid_ % (4096 * N6 * B6)
        x3 = int(uid_ / (4096 * N6 * B5)) - s
        uid_ = uid_ % (4096 * N6 * B5)
        y3 = int(uid_ / (4096 * N6 * B4)) - s
        uid_ = uid_ % (4096 * N6 * B4)
        x2 = int(uid_ / (4096 * N6 * B3)) - s
        uid_ = uid_ % (4096 * N6 * B3)
        y2 = int(uid_ / (4096 * N6 * B2)) - s
        uid_ = uid_ % (4096 * N6 * B2)
        x1 = int(uid_ / (4096 * N6 * B1)) - s
        uid_ = uid_ % (4096 * N6 * B1)
        y1 = int(uid_ / (4096 * N6)) - s
        uid_ = uid_ % (4096 * N6)
        o6 = int(uid_ / (4096 * N5))
        uid_ = uid_ % (4096 * N5)
        o5 = int(uid_ / (4096 * N4))
    ...
    def get_index(self, state):
        ...
        if uid is None:
            ...
            if index >= self.dim:
                return None
            elif self.lookup_tbl[index] == uid:
                return index
            else:
                return None
